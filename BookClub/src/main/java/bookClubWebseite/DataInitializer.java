...
public class DataInitializer {
   ...
   if(bookRepository.findByISBN("9783755800545") == null) {
           ...
           if(bookReaderRepository.findByUsername("superadmin") == null) {
            ...
            if(bookReaderRepository.findByUsername("user2") == null) {
                BookReader user = new BookReader();
                user.setUsername("user2");
                user.setEmail("usern2@example.com");
                user.setPassword(passwordEncoder.encode("user2")); // initiales Passwort
                user.setRole("USER"); // Rolle ADMIN
                bookReaderRepository.save(user);
                System.out.println("User user2 angelegt: user2 / user2");
            }              
